...
t=load(matstr(q).name);
      for i=5:numel(t.z) % i from 5 because I wanted to get rif of ...
first
...
% negative results one
  % for Rt and one for Rn which is not reasonable. The reason
% could be that
  % the ball drop for fall part was not done properly which
% caused negative
  % values as well as such a high values (over 1) in Rt
% coloumn. The rest of
  % values look okay. And the difference between the fall in
% which we used a
  % scarf and used no scarf is that Rn values are kind of
% lower for the ones
  % with scarf since it reduces the effect of impact.